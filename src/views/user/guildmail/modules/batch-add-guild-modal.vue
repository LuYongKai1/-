...
import { fetchImportGuildMail, fetchDownloadImportGuildMailTemplate } from "@/service/api";
import { handleApiResponseError, handleApiCatchError } from "@/utils/common";

defineOptions({
  name: "BatchAddGuildModal",
});
...
async function confirmUpload() {
  ...
  try {
    ...
    const response = await fetchImportGuildMail(formData);

    // 使用通用错误处理函数检查响应
    if (handleApiResponseError(response, "批量导入公会")) {
      return;
    }
    ...
    // 使用通用异常处理函数
    handleApiCatchError(error, "批量导入公会");
  } finally {
...
async function downloadTemplate() {
  try {
    const response = await fetchDownloadImportGuildMailTemplate();
