...
import { fetchImportRoleKick, fetchDownloadTemplateRoleKick } from "@/service/api";
import { handleApiResponseError, handleApiCatchError } from "@/utils/common";

defineOptions({
  name: "RoleKickImportModal",
});
...
async function downloadTemplate() {
  ...
  try {
    const response = await fetchDownloadTemplateRoleKick();
...
async function confirmImport() {
  ...
  try {
    ...
    const response = await fetchImportRoleKick(formData);

    if (handleApiResponseError(response, "导入踢出角色")) {
    ...
    handleApiCatchError(error, "导入踢出角色");
  } finally {
