...
import { fetchImportRoleBan, fetchDownloadTemplateRoleBan } from "@/service/api";
import { handleApiResponseError, handleApiCatchError } from "@/utils/common";

defineOptions({
  name: "RoleBanImportModal",
});
...
async function downloadTemplate() {
  ...
  try {
    const response = await fetchDownloadTemplateRoleBan();
...
async function confirmImport() {
  ...
  try {
    ...
    const response = await fetchImportRoleBan(formData);

    if (handleApiResponseError(response, "导入封禁角色")) {
    ...
    handleApiCatchError(error, "导入封禁角色");
  } finally {
