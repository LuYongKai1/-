...
import { fetchForceEditAnnouncement } from "@/service/api";
import { useFormRules, useNaiveForm } from "@/hooks/common/form";
import { handleApiResponseError, handleApiCatchError } from "@/utils/common";

defineOptions({
  name: "EditAnnouncementModal",
});
...
const rules = computed(() => ({
  guildNotice: [
    ...
    {
      max: 500,
      message: $t("page.manage.guildslist.announcementMaxLength"),
      trigger: ["input", "blur"],
    },
  ],
}));
...
async function handleSubmit() {
  await validate();
  try {
    const response: any = await fetchForceEditAnnouncement({
      guildId: model.value.guildGuid,
      serverId: props.guildData?.serverId,
      guildNotice: model.value.guildNotice,
    });

    // 使用统一的错误处理函数，如果有错误则返回
    if (handleApiResponseError(response, $t("common.editAnnouncement"))) {
      return;
    }

    // 成功后的处理
    window.$message?.success($t("common.editAnnouncementSuccess"));
    closeModal();
    emit("success");
  } catch (error: any) {
    // 表单验证错误不显示错误消息
    ...
    // 使用统一的异常处理函数
    handleApiCatchError(error, $t("common.editAnnouncement"));
  }
}
...
<template>
  <NModal v-model:show="visible" :title="$t('common.editAnnouncement')" preset="card" class="w-500px">
    <NForm
      ref="formRef"
      :model="model"
      :rules="rules"
    ...
    >
      ...
      <NFormItem :label="$t('page.manage.guildslist.guildAnnouncement')" path="guildNotice">
        <NInput
          v-model:value="model.guildNotice"
          type="textarea"
          :placeholder="$t('page.manage.guildslist.enterAnnouncement')"
          :maxlength="500"
          show-count
          clearable
          :rows="5"
