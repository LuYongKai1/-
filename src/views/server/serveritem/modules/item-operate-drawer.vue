...
import UpdateResultModal from "./update-result-modal.vue";
...
const currentOperationType = ref<'add' | 'edit'>('add');
...
const validateServerId = (
  ...
  if (numValue >= 10000) {
    return new Error($t("page.manage.serveritem.form.serverIdMax"));
  }
  return true;
};
...
const validateServerName = (
  ...
  if (strValue.length < 2) {
    return new Error($t("page.manage.serveritem.form.serverNameMinLength"));
  }

  // 检查第一个字符不能是数字或特殊符号
  // 只允许字母（a-zA-Z）、汉字（\u4e00-\u9fa5）、以及一些常见的其他语言字符
  const firstChar = strValue.charAt(0);
  if (!/^[a-zA-Z\u4e00-\u9fa5\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]/.test(firstChar)) {
    return new Error($t("page.manage.serveritem.form.serverNameInvalidStart"));
  }
...
const rules: FormRules = {
  ...
  serverId: [
    ...
    { validator: validateServerId, trigger: ["input", "blur"] },
  ],
  serverName: [
    {
      required: true,
    ...
    { validator: validateServerName, trigger: ["input", "blur"] },
  ],
  serverPort: [
    {
      required: true,
...
// 检查用户是否有修改"不可创建角色"的权限
const canModifyNoCreateRole = computed(() => {
  // 检查用户是否是管理员
  if (authStore.userInfo.user.admin) {
    return true;
  }
  // 检查用户是否有特定权限
  return authStore.userInfo.permissions.includes('server:noCreateRole:edit');
});
...
function createDefaultModel(): Model {
  return {
    ...
    maxUser: "5000", // 默认5000
    serverType: props.serverType || "NORMAL", // 使用 props 传入的值
    specId: null, // 默认无选中规格
    clusterStatus: "", // 添加缺失的集群状态字段
  };
}
...
async function getSpecOptions() {
  try {
    // 根据 serverType 获取对应类型的规格模板
    const specType = props.serverType || "NORMAL";
    const data: any = await fetchGetSpecList({
      specType: specType, // 根据 props.serverType 动态获取规格类型
    });
    ...
    if (rows && Array.isArray(rows)) {
      // 过滤只保留对应 specType 的规格
      const filteredRows = rows.filter((item: any) => item.specType === specType);
      specList.value = filteredRows;
      specOptions.value = filteredRows.map((item: Api.SystemManage.Spec) => ({
...
function formatTimestamp(timestamp: number | null): string | null {
  ...
  try {
    ...
    // 格式化为本地时间字符串，格式: YYYY-MM-DDTHH:mm:ss
  ...
  try {
    ...
    if (props.operateType === "edit" && props.rowData) {
      const {
        ...
        ...rest
      } = props.rowData as Api.SystemManage.serveritem & { updateDate?: string };
      ...
      const backendServerNew =
        ...
        // 处理 specId: 0 表示未选择规格的情况
        const rawSpecId = (rest as any).specId;
        const parsedSpecId = rawSpecId !== undefined && rawSpecId !== null && rawSpecId !== 0
          ? Number(rawSpecId)
          : null;
        ...
        model.value = {
          ...
          // 编辑模式下优先使用后端返回的 serverType，如果没有则使用 props.serverType
          serverType: (rest as any).serverType || props.serverType || "NORMAL",
          // 编辑模式下使用后端返回的值，如果为空则使用默认值
          urlCallback: (rest as any).urlCallback || "http://127.0.0.1:8864/mirm_api/service/payment",
          urlGm: (rest as any).urlGm || "http://127.0.0.1:30108/mirm_api/service/gm",
          extendParam: (rest as any).extendParam || "",
          // 从后端数据中获取规格信息，0 视为未选择
          specId: parsedSpecId,
        };
...
async function handleSubmit() {
  ...
  try {
    ...
    if (model.value.serverRecommendUI === "1") finalServerNew |= 2;
    if (model.value.serverHotUI === "1") finalServerNew |= 4;
    if (model.value.noCreateRoleUI === "1") finalServerNew |= 8;
    ...
    const {
      ...
      children,
      ...baseModelData
    } = model.value as Model & { createTime?: string; updateTime?: string; updateDate?: string; children?: unknown[] };
    ...
    let response;
    currentOperationType.value = props.operateType === "add" ? 'add' : 'edit';
    ...
    if (props.operateType === "add") {
      ...
      const msg = (response as any)?.response?.data?.msg || (response as any)?.msg || $t("common.addSuccess");
      ...
      const { createTime: _c, updateTime: _u, updateDate: _d, ...safePayload } = dataToSend as Record<string, unknown> & { createTime?: string; updateTime?: string; updateDate?: string };
      const updateData = { ...safePayload, id: props.rowData.id };
      ...
      const msg = (response as any)?.response?.data?.msg || (response as any)?.msg || $t("common.updateSuccess");
      resultMessage.value = msg;
    }
...
// 监听 serverType 变化，重新获取规格列表
watch(
  () => props.serverType,
  () => {
    if (visible.value) {
      getSpecOptions(); // 当 serverType 变化且弹窗打开时，重新获取规格列表
    }
  }
);
      ...
      >
        <NCollapse :default-expanded-names="['basic', 'spec']">
          ...
          <NCollapseItem name="spec">
            ...
            <NFormItem
              :label="$t('page.manage.serveritem.specTemplate')"
              path="specId"
            >
