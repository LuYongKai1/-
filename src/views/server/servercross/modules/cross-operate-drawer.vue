...
async function getSpecOptions() {
  try {
    ...
    if (data?.response?.data?.data && Array.isArray(data.response.data.data)) {
      ...
      Array.isArray(data.response.data.rows)
    ) {
      rows = data.response.data.rows;
    }

    if (rows && Array.isArray(rows)) {
      // 过滤只保留 specType 为 "CROSS" 的规格
      const filteredRows = rows.filter(
        (item: any) => item.specType === "CROSS"
      );
  ...
  try {
    ...
    if (props.operateType === "edit" && props.rowData) {
      ...
      model.value = {
        ...
        specId:
          (rest as any).specId !== undefined
            ? Number((rest as any).specId)
            : null,
      };
...
function selectSpec(specId: number) {
  ...
  if (selectedSpec) {
    model.value.maxUser =
      selectedSpec.maxUser !== undefined && selectedSpec.maxUser !== null
        ? String(selectedSpec.maxUser)
        : model.value.maxUser;
    model.value.busyUser =
      selectedSpec.busyUser !== undefined && selectedSpec.busyUser !== null
        ? String(selectedSpec.busyUser)
        : model.value.busyUser;
    model.value.fullUser =
      selectedSpec.fullUser !== undefined && selectedSpec.fullUser !== null
...
watch(
  ...
  ([busyUser, fullUser, maxUser]) => {
    ...
    // 使用 unref 确保获取最新的 ref 值，避免闭包陷阱
    const form = unref(formRef);
    if (!form) return;

    // 统一处理验证恢复逻辑
    let shouldRestoreValidation = false;

    if (busyUser && fullUser && maxUser) {
      shouldRestoreValidation = true;
    }
    ...
    // 使用 nextTick 确保在 DOM 更新后执行，避免闭包捕获过时的 ref
    if (shouldRestoreValidation) {
      nextTick(() => {
        // 在异步回调中重新获取最新的 formRef 值
      ...
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="top">
        <NCollapse :default-expanded-names="['basic', 'spec']">
          ...
          <NCollapseItem name="basic">
            <template #header>
              <span class="collapse-title">{{
                $t("page.manage.serveritem.basicInfo")
              }}</span>
            </template>
            ...
            <NGrid :cols="1" :x-gap="24">
              ...
              >
                <NDatePicker
                  ...
                  clearable
                  :placeholder="
                    $t('page.manage.serveritem.form.serverOpenDate')
                  "
                  style="width: 100%"
                />
              </NFormItemGi>
          ...
          <NCollapseItem name="spec">
            <template #header>
              <span class="collapse-title">{{
                $t("page.manage.serveritem.specTemplate")
              }}</span>
            </template>
                ...
                >
                  ...
                  <div class="spec-card-footer">
                    <span class="spec-code"
                      >{{ $t("page.manage.serveritem.specCode") }}:
                      {{ spec.specCode }}</span
                    >
                  </div>
                </div>
          ...
          <NCollapseItem name="network">
            <template #header>
              <span class="collapse-title">{{
                $t("page.manage.serveritem.networkConfig")
              }}</span>
            </template>
          ...
          <NCollapseItem name="address">
            <template #header>
              <span class="collapse-title">{{
                $t("page.manage.serveritem.addressConfig")
              }}</span>
            </template>
            <NGrid :cols="1" :x-gap="24">
              <NFormItemGi
          ...
          <NCollapseItem name="status">
            <template #header>
              <span class="collapse-title">{{
                $t("page.manage.serveritem.statusOptions")
              }}</span>
            </template>
