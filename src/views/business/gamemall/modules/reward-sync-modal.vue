...
import { fetchSyncMallTab } from "@/service/api";
...
async function confirmSync() {
  ...
  try {
    ...
    try {
      const response = await fetchSyncMallTab(serverIds);
      ...
      // 检查是否有错误
      const hasError = handleApiResponseError(response, `批量同步商城页签`);
    ...
    handleApiCatchError(error, '批量同步商城页签');
  } finally {
