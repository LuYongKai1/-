<script setup lang="ts">
  ...
  import { fetchImportMallTab } from "@/service/api";
  ...
  async function confirmImport() {
    ...
    try {
      ...
      // 调用导入发放商城API（批量导入所有选中的服务器）
      const response = await fetchImportMallTab(formData);

      // 使用通用错误处理函数检查响应
      if (handleApiResponseError(response, "导入发放商城")) {
        // 检查是否有错误文件需要下载
      ...
      // 使用通用异常处理函数
      handleApiCatchError(error, "导入发放商城");
    } finally {
