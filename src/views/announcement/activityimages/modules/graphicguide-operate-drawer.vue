...
defineOptions({
  name: "GraphicGuideOperateDrawer",
});
...
const title = computed(() => {
  const titles: Record<NaiveUI.TableOperateType, string> = {
    add: $t("page.manage.activityimages.addGraphicGuideImages"),
    edit: $t("page.manage.activityimages.editGraphicGuideImages"),
  };
...
type RuleKey = "name" | "servers" | "action";
...
async function handleSubmit() {
  ...
  try {
    ...
    const activityData: any = {
      ...
      sorter: model.value.sorter || 0,
      imageType: 2 // 固定为图文引导类型
    };
    ...
    if (props.operateType === "add") {
      response = await fetchAddActivityImages(activityData);
    } else {
      response = await fetchUpdateActivityImages(activityData);
    }

    const operationType = props.operateType === "add" ? "新增活动图片" : "修改活动图片";
    ...
    <NScrollbar class="h-530px pr-20px">
      ...
      >
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi
            span="24"
            :label="$t('page.manage.activityimages.action')"
            path="action"
          >
            <NInput
              v-model:value="model.action"
              :placeholder="$t('page.manage.activityimages.form.action')"
            />
          </NFormItemGi>
